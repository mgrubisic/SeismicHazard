function akZoomX(h_ax)

if strcmp(h_ax.XScale,'log')
    return
end

if strcmp(h_ax.YScale,'log')
    return
end
...
reset_button = 'alt';
cx = []; cy = []; mode = '';
original_xlim = h_ax.XLim;
original_ylim = h_ax.YLim;
zoom(h_fig, 'off')
pan(h_fig, 'off')
set(h_fig,'WindowScrollWheelFcn' , @scroll_zoom,...
    ...
    function scroll_zoom(varargin)
        ...
        if zoomFactor ~= 0 % could happen when fast scrolling
            ...
            [new_xlim(2),new_ylim(2)] = rel2absCoords(h_ax, new_xlim_rel(2), new_ylim_rel(2)); %#ok<*NASGU>
