function plot_PSDA_hazard(handles)
...
HazOptions = handles.HazOptions;
d = handles.d;

modo = HazOptions.mode;
...
switch modo
    case 1 % averaged displacment hazard
        ...
        if HazOptions.param(1)==1 % default weights
        ...
        if HazOptions.param(2)==1 % random weights
            weights = handles.HazOptions.rnd;
            %Nt      = length(weights);
        ...
        if HazOptions.param(3)==1 % percentiles
            percentile = HazOptions.param(4);
            Nt  = size(lambdaD,1);
        ...
        if HazOptions.param(5)==1 % displays source contribution
            Ny = size(lambdaD,1);
        ...
        if HazOptions.param(5)==0 % legend
    ...
    case 2 % single branch displacment
        ...
        if HazOptions.param(7)==1 % displays source contribution
        ...
        if HazOptions.param(8)==1 % displays source contribution
            branch_ptr = HazOptions.param(6);
...
% load MACEDO2 D_vector percentile_50_MC_example percentile_50_PC_example
% hold on
% plot(handles.ax1,D_vector,percentile_50_MC_example,'r.')
% plot(handles.ax1,D_vector,percentile_50_PC_example,'r.')
